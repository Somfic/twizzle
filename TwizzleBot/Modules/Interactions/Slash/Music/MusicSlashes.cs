...
public class MusicSlashes : InteractionModuleBase<SocketInteractionContext<SocketInteraction>>
{
    ...
    public MusicSlashes(AudioPlayer player, SpotifyGrabber spotify)
    {
        _player = player;
        _spotify = spotify;
    }
    ...
    [SlashCommand("spotify", "Plays music from Spotify", runMode: RunMode.Async)]
    public async Task Play([Summary(description: "A spotify link")]Uri link)
    {
        await DeferAsync();
