...
namespace TwizzleBot.Handlers.Interactions
{
    public class InteractionHandler : IHandler
    {
        ...
        public async Task Register(Assembly assembly)
        {
            ...
            if (_config.GetValue<bool>("Discord:IsDev"))
            {
                await _interactionService.RegisterCommandsToGuildAsync(_config.GetValue<ulong>("Discord:DevGuildId"));
                await _interactionService.RegisterCommandsToGuildAsync(790291218927058956); // ola ninion
            }
        }
